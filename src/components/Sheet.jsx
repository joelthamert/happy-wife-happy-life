...
const Sheet = ({ open, onClose, children }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "var(--scrim)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", zIndex: 300, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ ...glass({ borderRadius: "30px 30px 0 0" }), background: "var(--sheet-bg)", borderBottom: "none", padding: "18px 22px calc(42px + env(safe-area-inset-bottom))", width: "100%", maxWidth: 480, animation: "sheetUp .5s cubic-bezier(.16,1,.3,1)" }}>
